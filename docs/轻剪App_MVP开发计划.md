...
| D6 压缩三档预设（480p/720p/1080p + 进度 + 失败处理） | ✅ 已实现（随 D3-D4 内置，本轮核对补齐：失败清理半成品、输出大小显示） |
| D7 边界打磨（60s 录像上限+倒计时、权限拒绝引导、失败清理、图标） | ✅ 已实现（代码就绪，待真机验证） |

## 0.2 后续增强（2026-09-14 追加）

| 增强 | 状态 |
|---|---|
| E1 独立压缩入口：相册选视频 → 直接选预设压缩导出（补齐相册压缩闭环） | ✅ 已实现 |
| E2 倍速：0.5x / 1x / 1.5x / 2x（setpts + atempo） | ✅ 已实现 |
